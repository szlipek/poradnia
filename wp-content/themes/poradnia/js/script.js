    ...
    var $i = function (H) {
        ...
        if (d.forEach(function (t) {
            ...
            var N, L, B, S, K, J, U, _, Z, $ = H.autoWidth, tt = sn("fixedWidth"), et = sn("edgePadding"),
                ...
                pt = sn("controlsText"), ht = sn("nav"), mt = sn("touch"), yt = sn("mouseDrag"), gt = sn("autoplay"),
            ...
            $ || $e(Kt || _t), r && (Ot = r, Dt = "translate", o ? (Dt += F ? "3d(" : "3d(0px, ", kt = F ? ", 0px, 0px)" : ", 0px)") : (Dt += F ? "X(" : "Y(", kt = ")")), I && (V.className = V.className.replace("tns-vpfix", "")), function () {
                ...
                if (k && D) for (var a in k) {
                    ...
                    var r = k[a], i = "", o = "", u = "", l = "", s = "", c = $ ? null : sn("items", a),
                        f = sn("fixedWidth", a), d = sn("speed", a), v = sn("edgePadding", a), p = sn("autoHeight", a),
                        h = sn("gutter", a);
...
// links slide to element

const links = document.querySelectorAll('a[href^="#"]');
if (links) {
    links.forEach(function (item) {
        item.addEventListener('click', scrollAnchors);
    });
}


function scrollAnchors(e, respond) {
    const distanceToTop = function distanceToTop(el) {
        return Math.floor(el.getBoundingClientRect().top);
    };
    e.preventDefault();

    let targetID = (respond) ? respond.hash : this.hash;
...
if(document.querySelector('.faq__row')) {
    const titles = document.querySelectorAll('.faq__single h4');

    titles.forEach(function(i){
        i.addEventListener('click', toggleFaq);
    })
}

function toggleFaq() {
    this.closest('.faq__single').classList.toggle('active');
}