...
class RemoteContentFetcher implements \Psr\Log\LoggerAwareInterface
{
     ...
     */
    private $logger;
    public function __construct(\WPDeskFIVendor\Mpdf\Mpdf $mpdf, \Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->logger = $logger;
    }
    ...
    {
        $this->logger->debug(\sprintf('Fetching (cURL) content of remote URL "%s"', $url), ['context' => \WPDeskFIVendor\Mpdf\Log\Context::REMOTE_CONTENT]);
        $ch = \curl_init($url);
        ...
        if (\curl_error($ch)) {
            ...
            $this->logger->error($message, ['context' => \WPDeskFIVendor\Mpdf\Log\Context::REMOTE_CONTENT]);
            if ($this->mpdf->debug) {
                throw new \WPDeskFIVendor\Mpdf\MpdfException($message);
            }
        }
    ...
    {
        $this->logger->debug(\sprintf('Fetching (socket) content of remote URL "%s"', $url), ['context' => \WPDeskFIVendor\Mpdf\Log\Context::REMOTE_CONTENT]);
        // mPDF 5.7.3
        $timeout = 1;
        $p = \parse_url($url);
        $file = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($p, 'path', '');
        $scheme = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($p, 'scheme', '');
        $port = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($p, 'port', 80);
        $prefix = '';
        if ($scheme === 'https') {
            $prefix = 'ssl://';
            $port = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($p, 'port', 443);
        }
        $query = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($p, 'query', null);
        if ($query) {
            $file .= '?' . $query;
        }
        if (!($fh = @\fsockopen($prefix . $p['host'], $port, $errno, $errstr, $timeout))) {
            $this->logger->error(\sprintf('Socket error "%s": "%s"', $errno, $errstr), ['context' => \WPDeskFIVendor\Mpdf\Log\Context::REMOTE_CONTENT]);
            return \false;
        }
