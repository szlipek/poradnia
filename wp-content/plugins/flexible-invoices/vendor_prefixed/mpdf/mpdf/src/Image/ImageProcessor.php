...
namespace WPDeskFIVendor\Mpdf\Image;
...
use WPDeskFIVendor\Mpdf\File\StreamWrapperChecker;
use WPDeskFIVendor\Mpdf\Gif\Gif;
use WPDeskFIVendor\Mpdf\Language\LanguageToFontInterface;
use WPDeskFIVendor\Mpdf\Language\ScriptToLanguageInterface;
use WPDeskFIVendor\Mpdf\Log\Context as LogContext;
use WPDeskFIVendor\Mpdf\Mpdf;
use WPDeskFIVendor\Mpdf\Otl;
use WPDeskFIVendor\Mpdf\RemoteContentFetcher;
...
class ImageProcessor implements \Psr\Log\LoggerAwareInterface
{
    ...
    public function getImage(&$file, $firsttime = \true, $allowvector = \true, $orig_srcpath = \false, $interpolation = \false)
    {
        /**
        ...
        if ($firsttime && isset($this->failedImages[$file])) {
        ...
        if ($type === 'png') {
            ...
            if ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
                ...
                // the alpha channel is sent through as secondtime as Indexed and should not be converted to CMYK
                // Convert to Grayscale image stream - nominally returned as type='png'
                $info = $this->convertImage($data, $colspace, 'DeviceGray', $w, $h, $ppUx, $pngalpha, $gamma, $ct);
                // mPDF 5.7.2 Gamma correction
            } elseif (($this->mpdf->PDFA || $this->mpdf->PDFX) && $pngalpha) {
                // Remove alpha channel
                if ($this->mpdf->restrictColorSpace === 1) {
                    // Grayscale
                    $info = $this->convertImage($data, $colspace, 'DeviceGray', $w, $h, $ppUx, $pngalpha, $gamma, $ct);
                    // mPDF 5.7.2 Gamma correction
                    ...
                    // RGB
                    $info = $this->convertImage($data, $colspace, 'DeviceRGB', $w, $h, $ppUx, $pngalpha, $gamma, $ct);
                    // mPDF 5.7.2 Gamma correction
                }
                if ($this->mpdf->PDFA && !$this->mpdf->PDFAauto || $this->mpdf->PDFX && !$this->mpdf->PDFXauto) {
