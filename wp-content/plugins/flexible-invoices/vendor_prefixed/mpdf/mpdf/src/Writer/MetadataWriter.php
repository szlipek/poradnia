...
class MetadataWriter implements \Psr\Log\LoggerAwareInterface
{
     ...
     */
    private $logger;
    public function __construct(\WPDeskFIVendor\Mpdf\Mpdf $mpdf, \WPDeskFIVendor\Mpdf\Writer\BaseWriter $writer, \WPDeskFIVendor\Mpdf\Form $form, \WPDeskFIVendor\Mpdf\Pdf\Protection $protection, \Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->writer = $writer;
        $this->form = $form;
    ...
    public function writeOutputIntent()
    {
        $this->writer->object();
    ...
    public function writeAssociatedFiles()
    {
        ...
        foreach ($this->mpdf->associatedFiles as $k => $file) {
            // spec
            $this->writer->object();
            $this->mpdf->associatedFiles[$k]['_root'] = $this->mpdf->n;
     ...
     * @since 5.7.2
     */
    public function writeAnnotations()
    {
        $nb = $this->mpdf->page;
        for ($n = 1; $n <= $nb; $n++) {
    ...
    public function writeEncryption()
    {
