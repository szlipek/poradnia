...
class Checkout implements \WPDeskFIVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $vat_number = isset($_POST['billing_vat_number']) ? \trim(\sanitize_text_field(\wp_unslash($_POST['billing_vat_number']))) : 'en';
        // phpcs:ignore
    ...
    public function save_wpml_user_session_lang($order_id)
    {
        ...
        // phpcs:ignore
        if ($wpml_user_lang) {
            \update_post_meta($order_id, 'wpml_user_lang', $wpml_user_lang);
        }
    }
}
