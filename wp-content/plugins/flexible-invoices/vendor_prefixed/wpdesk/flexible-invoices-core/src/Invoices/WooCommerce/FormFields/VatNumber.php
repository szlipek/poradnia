...
class VatNumber extends \WPDeskFIVendor\WPDesk\Library\FlexibleInvoicesCore\WooCommerce\FormFields\FormField
{
    ...
    protected function prepare_checkout_field($field_priority = null) : array
    {
        ...
        return ['label' => $this->label, 'placeholder' => $this->placeholder, 'required' => $this->get_required(), 'class' => \is_admin() ? '' : ['form-row-wide'], 'clear' => \true, 'priority' => $field_priority, 'default' => $default ?? ''];
    }
    /**
     * Prepare admin field.
     *
