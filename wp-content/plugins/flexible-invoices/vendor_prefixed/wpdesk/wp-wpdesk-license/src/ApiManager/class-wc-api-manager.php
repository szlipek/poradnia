...
use WPDeskFIVendor\WPDesk\Notice\AjaxHandler;
use WPDeskFIVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('WPDeskFIVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
         ...
         */
        public $text_domain = 'flexible-invoices';
        /**
         * @var string
         */
        public $plugin_url;
        ...
        private function add_not_possible_update_message()
        {
            \add_action('in_plugin_update_message-' . $this->plugin_name, function (array $plugin_data, \stdClass $response) {
                if (isset($response, $response->package) && empty($response->package)) {
                    echo \esc_html(\sprintf(\__('%1$sEnter a valid subscription key for automatic updates.', 'flexible-invoices'), ' '));
                }
        ...
        {
            $password_management = new \WPDeskFIVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \WPDeskFIVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \WPDeskFIVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function inactive_notice()
        {
            ...
            if (\apply_filters('wpdesk_show_plugin_activation_notice', \true)) {
                (new \WPDeskFIVendor\WPDesk\Notice\AjaxHandler())->hooks();
                new \WPDeskFIVendor\WPDesk\Notice\PermanentDismissibleNotice(\sprintf(\__('The %1$s%2$s%3$s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %4$sClick here%5$s to activate the API key and the plugin.', $this->text_domain), '<strong>', $this->plugin_title, '</strong>', '<a href="' . \esc_url(\admin_url('plugins.php#' . $this->plugin_dir . '-wpdesk-license')) . '">', '</a>'), "notice-{$this->software_product_id}", \WPDeskFIVendor\WPDesk\Notice\PermanentDismissibleNotice::NOTICE_TYPE_WARNING);
            }
        }
        /**
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('WPDeskFIVendor\\WP_HTTP_BLOCK_EXTERNAL') && \WPDeskFIVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('WPDeskFIVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\WPDeskFIVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
        ...
        {
            $parser = new \WPDeskFIVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \WPDeskFIVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \WPDeskFIVendor\WPDesk\License\Changelog\Formatter($changes);
