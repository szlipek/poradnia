...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'flexible-invoices'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'flexible-invoices'), 'product_status' => \__('Subscription Status', 'flexible-invoices'), 'product_license' => \__('Subscription Data', 'flexible-invoices')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'flexible-invoices');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'flexible-invoices');
        }
        return $status;
    }
