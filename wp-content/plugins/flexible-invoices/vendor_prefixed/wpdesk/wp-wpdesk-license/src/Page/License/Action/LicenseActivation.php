...
use WPDeskFIVendor\WPDesk\License\ServerAddressRepository;
 ...
 */
class LicenseActivation extends \WPDeskFIVendor\WPDesk\License\Page\License\Action\AbstractAction implements \WPDeskFIVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \WPDeskFIVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        $this->add_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-invoices'), 'error');
    }
    /**
    ...
    {
        $this->add_error('activate_text', 'activate_msg', \__('Plugin license activated. ', 'flexible-invoices') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \WPDeskFIVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
