...
use WPDeskFIVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use WPDeskFIVendor\WPDesk\Logger\WC\WooCommerceCapture;
use WPDeskFIVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \WPDeskFIVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\WPDeskFIVendor\Monolog\Registry::hasLogger($name)) {
            \WPDeskFIVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\WPDeskFIVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \WPDeskFIVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \WPDeskFIVendor\Monolog\Logger($name);
        }
        if (\WPDeskFIVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \WPDeskFIVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\WPDeskFIVendor\Monolog\Logger $logger)
    {
        if (!\defined('WPDeskFIVendor\\WC_LOG_THRESHOLD')) {
            \define('WPDeskFIVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \WPDeskFIVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \WPDeskFIVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \WPDeskFIVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \WPDeskFIVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\WPDeskFIVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \WPDeskFIVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \WPDeskFIVendor\Monolog\Registry::hasLogger($name);
    }
}
